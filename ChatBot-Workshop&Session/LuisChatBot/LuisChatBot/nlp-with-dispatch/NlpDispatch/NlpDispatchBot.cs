...
namespace NLP_With_Dispatch_Bot
{
    ...
    public class NlpDispatchBot : IBot
    {
        ...
        private const string travelagent = "travelagent";
        ...
        public NlpDispatchBot(BotServices services)
        {
            ...
            if (!_services.LuisServices.ContainsKey(travelagent))
            {
                throw new System.ArgumentException($"Invalid configuration. Please check your '.bot' file for a Luis service named '{travelagent}'.");
            }
        }
        ...
        private async Task DispatchToTopIntentAsync(ITurnContext context, (string intent, double score)? topIntent, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            const string travelAgentDispatchKey = "I_TravelAgent";
            ...
            switch (topIntent.Value.intent)
            {
                ...
                case weatherDispatchKey:
                    ...
                    break;
                case travelAgentDispatchKey:
                    await DispatchToLuisModelAsync(context, travelagent);
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                await context.SendActivityAsync(
                    $"Intents detected by the {appName} app:\n\n{string.Join("\n\n", result.Intents)}");

                if (result.Entities.Count > 0)
                {
                    await context.SendActivityAsync(
                        $"The following entities were found in the message:\n\n{string.Join("\n\n", result.Entities)}");
                }
            }
            catch (Exception e)
            {
